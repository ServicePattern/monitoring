...
def hc_update(hc_key):
    logger = logging.getLogger()
    response = requests.get("https://hc.brightpattern.com/ping/"+hc_key+"/log-archive")
...
# Gets sent daily
def email_summary(recipients, channel="email", teams_url=''):
    successful_jobs = 0
    total_jobs = 0
...
def main(args):
    ...
    try:
        ...
        if not success:
            ...
            # sending email summaery to teams channel in any case
            email_summary(recipients=recipients, channel='teams', teams_url=teams_url)
            # update healthchecks
            hc_update(hc_key)
