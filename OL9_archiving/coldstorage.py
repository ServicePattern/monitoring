...
def send_email(recipients=[], subject='', message='', attachment=None):
    ...
    try:
        tls = conf.get('DEFAULT', 'smtp-tls')
    except configparser.NoOptionError :
        logger.warn("No tls value defined in the config")
        tls=False
    try:
        smtp_user = conf.get('DEFAULT', 'smtp-user')
    except configparser.NoOptionError :
        logger.warn("No smtp-user value defined in the config")
        smtp_user=None
    try:
        smtp_pass = conf.get('DEFAULT', 'smtp-pass')
    except configparser.NoOptionError :
        logger.warn("No smtp-pass value defined in the config")
        smtp_pass=None
...
def send_teams(subject='', message='', webhook=''):
    logger = logging.getLogger()
    hostname = get_hostname()
    short_hostname = hostname.split('.', 1)[0]
    msg = subject.format("**"+short_hostname+"**") + "\n{0}".format(message)
    payload = { "text": msg }
    headers = {'Content-Type': 'application/json' }
    response = requests.post(webhook, headers=headers, data=json.dumps(payload))

def hc_update(hc_key):
    logger = logging.getLogger()
    response = requests.get("https://hc.brightpattern.com/ping/"+hc_key+"/deep-archive")
...
def teams_failure(err_msg, teams_url):
    send_teams(
        subject = 'Log backup failed: {0}',
        message = "\n{0}".format(err_msg),
        webhook = teams_url)

def teams_success(teams_url):
    try:
...
def get_client(region_name='us-east-1', service='s3', profile_name="default"):
    session = get_session(region_name=region_name, profile_name=profile_name)
    client = session.client(service)
    return client


def get_file_s3(bucket, key, aws_region, aws_profile):
    s3 = get_client(region_name=aws_region, service='s3', profile_name=aws_profile )
...
def extract_catalog_data(catalog):
    ...
    if catalog:
        for item in catalog['Body'].read().splitlines():
            catalog_data.append(item.strip().decode())
...
def upload_catalog(catalog_data, bucket, s3_prefix, target, aws_profile):
    logger = logging.getLogger()
    ...
    try:
        ...
        cmd = 'aws --profile {2} s3 cp {0} {1}'.format(source, destination, aws_profile )
...
def coldstorage(args, conf):
    ...
    hc_key = conf.get('DEFAULT', 'healthcheck-key')
    teams_url = conf.get('DEFAULT', 'teams-url')
    aws_profile = conf.get('DEFAULT', 'aws_profile')
    aws_region = conf.get('DEFAULT', 'aws_region')
    # --------------------------------------------------------------------------
    ...
    for target in targets:
        ...
        try:
            ...
            catalog = get_file_s3(bucket, key, aws_region, aws_profile)
        ...
        try:
            for archive in new_local_archives:
                ...
                cmd = 'aws --profile {3} s3 sync {0} {1} --storage-class {2}'.format(source, destination, storage_class, aws_profile)
            ...
            upload_catalog(catalog_data, bucket, s3_prefix, target, aws_profile)
            logger.info('====================================================')
...
#         email_success(recipients=recipients)
        teams_success(teams_url=teams_url)
        hc_update(hc_key)
    except Exception as e:
        ...
        teams_failure(err_msg=str(e), teams_url=teams_url)
