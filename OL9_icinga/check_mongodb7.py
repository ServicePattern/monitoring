...
from pymongo.errors import ConnectionFailure, PyMongoError, AutoReconnect
import ssl as SSL
import sys
...
def mongo_connect(host=None, port=None, ssl=False, user=None, passwd=None, replica=None, authdb="admin", insecure=False, ssl_ca_cert_file=None, ssl_cert=None, auth_mechanism=None, retry_writes_disabled=False):
    ...
    # Disable retry writes if specified
    if retry_writes_disabled:
        con_args['retryWrites'] = False

    # Validate authMechanism or set a default
    supported_mechanisms = ('PLAIN', 'DEFAULT', 'SCRAM-SHA-1', 'GSSAPI', 'MONGODB-X509', 'SCRAM-SHA-256', 'MONGODB-AWS', 'MONGODB-CR')
    if not auth_mechanism:
        auth_mechanism = 'SCRAM-SHA-256'  # Set a default auth mechanism
    elif auth_mechanism not in supported_mechanisms:
        print(f"CRITICAL - Unsupported authMechanism: {auth_mechanism}. Must be one of {supported_mechanisms}")
        sys.exit(2)

    try:
        # Connection setup with authentication
        if replica is None:
            con = MongoClient(host, port, username=user, password=passwd, authSource=authdb, authMechanism=auth_mechanism, **con_args)
        else:
            con = MongoClient(host, port, username=user, password=passwd, authSource=authdb, authMechanism=auth_mechanism, replicaSet=replica, read_preference=ReadPreference.SECONDARY, **con_args)

        # Check if the connection is successful and determine if it's an arbiter
        try:
            result = con.admin.command("ismaster")
        except ConnectionFailure:
            print(f"CRITICAL - Connection to Mongo server on {host}:{port} has failed")
            sys.exit(2)

        if 'arbiterOnly' in result and result['arbiterOnly'] == True:
            print(f"OK - State: 7 (Arbiter on port {port})")
            sys.exit(0)
        ...
        if isinstance(e, AutoReconnect) and "is an arbiter" in str(e):
            print("OK - State: 7 (Arbiter)")
            sys.exit(0)
        print(f"CRITICAL - General MongoDB Error: {e}")
